...
import { useAuthenticator } from '@aws-amplify/ui-react'; // <--- IMPORTS USER HOOK
import DashboardView from './DashboardView'; // Your Charts Component
import PoliciesView from './PoliciesView';   // Your List Component

export default function Dashboard() {
    // 1. Get the current logged-in user (Fixes ReferenceError: user)
    const { user, signOut } = useAuthenticator((context) => [context.user]);

    // 2. State to toggle views
        ...
        <div className="flex h-screen bg-slate-900 text-slate-200 font-sans overflow-hidden">
            ...
            <main className="flex-1 overflow-auto bg-slate-900 relative">
                {/* 3. CONDITIONAL RENDERING
                   Here we pass 'userId' to PoliciesView. 
                   The 'user' variable is now defined from the hook above.
                */}
                {currentView === 'dashboard' && <DashboardView />}
                
                {currentView === 'policies' && (
                    <PoliciesView 
                        userId={user?.userId || user?.username} // Safe access
